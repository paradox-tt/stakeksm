...
function createKeyRaw(registry, itemFn, keys, hashers, args) {
  ...
  return (0, _util.u8aConcat)(createPrefixedKey(itemFn), ...keys.map((type, index) => {
    const arg = args[index];
    (0, _util.assert)(!(0, _util.isUndefined)(arg) && !(0, _util.isNull)(arg), () => `Call to ${(0, _util.stringCamelCase)(section || 'unknown')}.${(0, _util.stringCamelCase)(method || 'unknown')} has a null or undefined argument at position ${index}`);
...
function createKey(registry, itemFn, keys, hashers, args) {
  ...
  (0, _util.assert)(Array.isArray(args) && args.length === keys.length, () => `Call to ${(0, _util.stringCamelCase)(section || 'unknown')}.${(0, _util.stringCamelCase)(method || 'unknown')} needs ${keys.length} arguments, found ${(0, _util.stringify)(args)}`); // as per createKey, always add the length prefix (underlying it is Bytes)
...
function extendPrefixedMap(registry, itemFn, storageFn) {
  ...
  storageFn.iterKey = extendHeadMeta(registry, itemFn, storageFn, (...args) => {
    (0, _util.assert)(args.length === 0 || type.isDoubleMap && args.length === 1 || type.isNMap && args.length === type.asNMap.hashers.length - 1, () => `Iteration ${(0, _util.stringCamelCase)(section || 'unknown')}.${(0, _util.stringCamelCase)(method || 'unknown')} needs arguments to be one less than the full arguments, found [${args.join(', ')}]`);

    if (args.length) {
      if (type.isDoubleMap) {
        return new _codec.Raw(registry, createKeyRaw(registry, itemFn, [type.asDoubleMap.key1], [(0, _getHasher.getHasher)(type.asDoubleMap.hasher)], args));
      } else if (type.isNMap) {
        const keys = [...type.asNMap.keyVec];
        const hashers = type.asNMap.hashers.map(h => (0, _getHasher.getHasher)(h)); // remove the last entry

        keys.pop();
        hashers.pop();
        return new _codec.Raw(registry, createKeyRaw(registry, itemFn, keys, hashers, args));
      }
    }

    return new _codec.Raw(registry, createKeyRaw(registry, itemFn, [], [], []));
  });
  return storageFn;
}
/** @internal */
...
function createFunction(registry, itemFn, options) {
  ...
  const storageFn = expandWithMeta(itemFn, arg => type.isPlain ? options.skipHashing ? (0, _util.compactAddLength)((0, _util.u8aToU8a)(options.key)) : createKey(registry, itemFn, [], [], []) : type.isMap ? createKey(registry, itemFn, [type.asMap.key], [(0, _getHasher.getHasher)(type.asMap.hasher)], [arg]) : type.isDoubleMap ? createKey(registry, itemFn, [type.asDoubleMap.key1, type.asDoubleMap.key2], [(0, _getHasher.getHasher)(type.asDoubleMap.hasher), (0, _getHasher.getHasher)(type.asDoubleMap.key2Hasher)], arg) : createKey(registry, itemFn, type.asNMap.keyVec, type.asNMap.hashers.map(h => (0, _getHasher.getHasher)(h)), arg));
