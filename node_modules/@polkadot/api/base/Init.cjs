...
class Init extends _Decorate.Decorate {
  ...
  async getBlockRegistry(blockHash) {
    ...
    const metadata = new _metadata.Metadata(this.registry, await this._rpcCore.state.getMetadata.raw(header.parentHash).toPromise());

    const registry = this._initRegistry(new _create.TypeRegistry(blockHash), this._runtimeChain, version, metadata); // add our new registry
  ...
  _detectCapabilities(registry, blockHash) {
    (0, _capabilities.detectedCapabilities)(this._rx, blockHash).toPromise().then(types => {
